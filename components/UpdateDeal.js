...
const UpdateDeal = ({ org, handleOnClick, editDeal }) => {
  ...
  const fetchDeal = async () => {
    const response = await axios.get(`/api/catalog/get-deal?id=${editDeal.id}`);
  ...
  // 🔑 when deal is loaded, sync into form states
  useEffect(() => {
    if (deal) {
      setTitle(deal.title || "");
      setDescription(deal.description || "");
      setType(deal.type || "ONE_OFF");
      setStartDate(deal.startDate ? deal.startDate.substring(0, 10) : "");
      setEndDate(deal.endDate ? deal.endDate.substring(0, 10) : "");
      setRecurrence(
        deal.recurrence && Array.isArray(deal.recurrence)
          ? deal.recurrence
          : daysOfWeek.map((d) => ({ day: d, enabled: false, timeStart: "", timeEnd: "" }))
      );
      ...
      setImage(deal.imageUrl ? [deal.imageUrl] : []);
      setIsActive(deal.isActive ?? true);
    }
  }, [deal]);

  // fetch all data when editDeal changes
  useEffect(() => {
    if (editDeal?.id) {
      fetchInventories();
      fetchProducts();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/catalog/update-deal`, {
        method: "PUT",
  ...
  return (
    <div className="form_container" style={{ width: "70%" }}>
      <form onSubmit={handleSubmit}>
        ...
        {!loading && <button className="techwave_fn_button" type="submit">Update Deal</button>}
        <br />
